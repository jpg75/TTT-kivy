...
class TTTGame(AnchorLayout):
    ...
    def relocate(self, card, other_card):
        """Set the new zone for the current and companion card and set up the
        animations for both cards.
        """
        card_temp_x = card.old_x
        card_temp_y = card.old_y
        other_card_temp_x = other_card.x
        other_card_temp_y = other_card.y
        ...
        anim1 = Animation(pos=(other_card_temp_x, other_card_temp_y), t='in_quad', d=0.5)
        anim2 = Animation(pos=(card_temp_x, card_temp_y), t='in_quad', d=0.5)
        anim1.start(card)
        anim2.start(other_card)
        card.zone = other_card.zone
        ...
        if run_index < len(self.shoe_config.content):  # hand available
            ...
            for c in cards:
                if c.zone == 'Color Position':
                    ...
                    print "Warning: unknown position '%s'!" % c.zone
    ...
    def on_touch_down(self, touch):
        ...
        if self.collide_point(touch.x, touch.y):
            ...
            if ((self.zone == 'Color Position' and self.parent.parent.parent.current_player == 'ck') \
                        or (self.zone == 'Number Position' and self.parent.parent.parent.current_player == 'nk') or (
                    self.zone == 'Color Position' and self.parent.parent.parent.current_player == 'ck') \
